...
def find_s(from_start, line):
    func = min if from_start else max
    digits = list(
        filter(
            lambda i: i[0] >= 0,
            [
                (line.find(x) if from_start else line.rfind(x), str_to_int[x])
                for x in str_digits
            ],
        )
    ) + [(9999 if from_start else -1, 0)]
    return func(digits, key=lambda x: x[0])


def find_d(from_start, line):
    func = min if from_start else max
    digits = list(
        filter(
            lambda i: i[0] >= 0,
            [
                (line.find(x) if from_start else line.rfind(x), int(x))
                for x in "123456789"
            ],
        )
    ) + [(9999 if from_start else -1, 0)]
...
for line in data.strip().split():
    s1 = find_s(True, line)
    d1 = find_d(True, line)
    s2 = find_s(False, line)
    d2 = find_d(False, line)

    dx = min([s1, d1], key=lambda x: x[0])[1]
    dy = max([s2, d2], key=lambda x: x[0])[1]
