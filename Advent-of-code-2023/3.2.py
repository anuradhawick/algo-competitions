...
def get_gear_locations(r, c):
    gears = []
    for r_op in [-1, 0, 1]:
        for c_op in [-1, 0, 1]:
            try:
                char = matrix[r + r_op][c + c_op]
                if char == "*":
                    gears.append((r + r_op, c + c_op))
            except:
                pass
...
for r, row in enumerate(matrix):
    ...
    for c, char in enumerate(row):
        if char.isnumeric():
            number += char
            gear_locations.update(get_gear_locations(r, c))
        else:
            if len(number) and len(gear_locations):
                for loc in gear_locations:
                    gear_neighbours[loc].append(number)
            number = ""
            gear_locations = set()
    if len(number) and len(gear_locations):
        for loc in gear_locations:
...
for k, v in gear_neighbours.items():
    if len(v) == 2:
        v = list(map(int, v))
        s += v[0] * v[1]
