...
def parse_colors(content: str):
    takes = [
        defaultdict(int, {
            colour.strip().split()[1]: int(colour.strip().split()[0])
            for colour in take.strip().split(",")
        })
        for take in content.strip().split(";")
    ]
    return takes


def compute_min_game(games: list[dict]):
    result = defaultdict(int)
    for game in games:
        for col in ['red', 'green', 'blue']:
            result[col] = max(result[col], game[col])
    return result


games = {
    int(line.split(":")[0].replace("Game ", "")): parse_colors(line.split(":")[1])
    for line in data.strip().split("\n")
}

def validate(game: dict):
...
for k, v in games.items():
    s += reduce(mul, compute_min_game(v).values())
